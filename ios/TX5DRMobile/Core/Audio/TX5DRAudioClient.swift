...
final class TX5DRAudioClient: ObservableObject {
    ...
    init(urlSession: URLSession = TX5DRNetworkPolicy.webSocketSession) {
    ...
    private func handleDownlinkAudio(_ data: Data) throws {
        ...
        // TODO(latency): this drops the *newest* frame when the queue is full,
        // which pins latency at the cap instead of recovering from it. Dropping
        // the oldest (or resyncing the player node) would be better for a
        // conversation, where being three seconds behind is worse than one
        // audible gap. Deliberately left for a follow-up: it needs a real-device
        // measurement of `scheduledPlaybackFrames` under drift, which is only
        // meaningful now that the counter above is accurate.
        ...
        //
        // The legacy handler fires with `.dataConsumed` semantics — when the
        // player has *taken* the buffer, not when it has been heard. With that
        // callback `scheduledPlaybackFrames` decrements early, so the
        // `maximumPlaybackFrames` guard above never actually bites and the
        // player node's own queue grows without bound. The radio's USB codec
        // clock and the phone's clock always differ by some ppm; whenever the
        // radio runs faster the backlog then grows monotonically, which is how
        // receive audio ends up seconds behind even on a LAN.
