...
final class TX5DRSession: ObservableObject {
    ...
    /// Releases PTT when the system takes the microphone away (incoming
    /// call, Siri, media-services reset, or the capture device being
    /// unplugged). `scenePhase` cannot do this job: it also fires for the
    /// microphone permission alert, which would cancel the operator's very
    /// first PTT before they can allow it.
    private var audioInterruptionObserver: TX5DRAudioInterruptionObserver?
    ...
    init() {
        ...
        audioInterruptionObserver = TX5DRAudioInterruptionObserver { [weak self] in
            self?.handleAudioInterruption()
        }
    }

    /// Never resumes anything: an interruption only ever takes the station off
    /// the air. Coming back is an explicit operator action.
    private func handleAudioInterruption() {
        guard isVoicePTTHeld else { return }
        noticeMessage = "音频被系统中断，已松开 PTT"
        endVoicePTT()
    }
    ...
    func beginVoicePTT() {
        ...
        isVoicePTTHeld = true
        audioInterruptionObserver?.rearm()
        pttGeneration += 1
