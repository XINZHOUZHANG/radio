...
enum TX5DRNetworkPolicy {
    ...
    ///
    /// This used to be five minutes with `waitsForConnectivity`, which turned
    /// "the saved address is unreachable" into a silent five-minute spinner:
    /// on a LAN with Tailscale off, the stored `100.x` address is routable
    /// nowhere, and URLSession treats that as "waiting for connectivity"
    /// rather than failing. A remote-control app has to say "cannot reach the
    /// server" quickly so the operator can change the address.
    static let requestTimeout: TimeInterval = 15

    /// Ceiling for a whole transfer: ADIF import/export and plugin assets can
    /// legitimately run long even though each round trip must not.
    static let resourceTimeout: TimeInterval = 300

    /// WebSocket idle ceiling. `RadioWebSocket` sends an application-level
    /// ping every 25 s and `TX5DRAudioClient` pings every 20 s, so a socket
    /// that has been silent for a minute is dead, not slow.
    static let webSocketIdleTimeout: TimeInterval = 60

    static let maximumWebSocketMessageSize = 64 * 1024 * 1024

    /// REST session. `waitsForConnectivity` is deliberately off — see
    /// `requestTimeout`.
    ...
    /// Long-lived control, logbook and audio sockets. These must not inherit
    /// the REST per-request deadline: a quiet control channel is normal.
    static let webSocketSession: URLSession = {
        let configuration = URLSessionConfiguration.default
        configuration.waitsForConnectivity = false
        configuration.timeoutIntervalForRequest = webSocketIdleTimeout
        configuration.timeoutIntervalForResource = resourceTimeout
        return URLSession(configuration: configuration)
    }()
    ...
    static func webSocketTask(session: URLSession, url: URL) -> URLSessionWebSocketTask {
        var request = URLRequest(url: url)
        request.timeoutInterval = webSocketIdleTimeout
